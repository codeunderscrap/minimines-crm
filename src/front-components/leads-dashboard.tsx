...
const LeadsDashboard = () => {
  ...
  useEffect(() => { loadData(); setTimeout(() => { if (typeof window !== 'undefined' && window.sessionStorage) window.sessionStorage.removeItem('urlHighlightId'); }, 2000); }, []);
            ...
            {visibleLeads.map(lead => {
              ...
              return (
                ...
                <div id={`lead-row-${lead.id}`} onClick={() => setExpandedLeadId(expandedLeadId === lead.id ? null : lead.id)} style={{
                  ...
                  <div>
                    <div style={{ fontWeight: 600, color: BRAND.primary, fontSize: '14px' }}><a href={`/object/lead/${lead.id}`} style={{ textDecoration: "none", color: "inherit", fontWeight: "bold" }}>{lead.name}</a></div>
